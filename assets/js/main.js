$(document).ready(function() {
  ...
  $('#item_name').on('change', function(){
      ...
      $.ajax({
          ...
          success: function(result){
            ...
            $.each( result, function( key, value ) {
              ...
              select_str += "<option value='"+contractor_id+"'>"+contractor_name+"</option>";
            });
    ...
    if((itemType == 'receipt' && parseInt(itemName) == 3) || itemType == 'expense' && parseInt(itemName) == 6){
      $('#block_official').toggle();
      $('#block_description').toggle();
      $('#block_sum').toggle();
      $('#block_document').toggle();
      ...
      $.ajax({
          ...
          success: function(result){
            ...
            $.each( result, function( key, value ) {
              ...
              select_str += "<option value='"+value.id+"' agreement_sum='"+value.agreement_sum+"'  agreement_type = '"+value.type+"'>"+agreementName+"("+short_name+")"+"</option>";
            });
  ...
  $('a[name="history_deletion"]').on('click', function(){
    ...
    $.ajax({
        ...
        success: function(result){
          ...
          var modal_history = new bootstrap.Modal(document.getElementById('modal_history'), {keyboard: false});
          var agreement = "";
          var agreement_date = "";
          ...
          var history =
          "Наименование статьи: "+result[0].item_name+"<br>"+
          "Договор: №"+agreement+" от "+agreement_date+"<br>"+
          "Описание: "+result[0].description+"<br>"+
          "Сумма: "+result[0].sum;
          $('#history_item').html(history);
          $('#history_reason').text(result[0].status_reason);
          $('#history_edition_name').html("<i class='fa-solid fa-trash-can'></i> Удаление ");
          $('#history_author').text(result[0].emp_surname+" "+result[0].emp_name);
  ...
  $('a[name="history_edition"]').on('click', function(){
    ...
    $.ajax({
        ...
        success: function(result){
          ...
          $('#modal_history_form').attr('action', base_url+'/dashboard/approve_edition');
          var history =
          "Наименование статьи: "+result[0].item_name+"<br>"+
          "Договор: №"+agreement+" от "+agreement_date+"<br>"+
          "Описание: "+result[0].description+"<br>"+
          "Сумма: "+result[0].sum+"<br>"+
          "Документ: "+result[0].document+"<br><hr>"+
          "Новая сумма: "+result[0].edition_new_value+"<br>"+
          "Новый документ: <a href='"+base_url+"/uploads/"+result[0].edition_doc+"'>"+result[0].edition_doc+"</a>";
          $('#history_item').html(history);
          $('#history_reason').text(result[0].edition_reason);
          $('#history_edition_name').html("<i class='fa-solid fa-pen-to-square'></i> Редактирование ");
          $('#history_author').text(result[0].emp_surname+" "+result[0].emp_name);
          $('#aprove_item_type').val(item_type);
          $('#aprove_record_id').val(record_id);
